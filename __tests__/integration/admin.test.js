...
describe("admin service", () => {
  ...
  describe("getBestClients", () => {
    ...
    beforeAll(async () => {
      ...
      pepeNunesClient = await Profile.create(
        generateProfile({
          type: "client",
          firstName: "Pepe",
          lastName: "Nunes",
      ...
      pepeNunesContract = await Contract.create(
        generateContract({
          ClientId: pepeNunesClient.id,
          ContractorId: contractor.id,
        })
      );
      ...
      pepeBotellaPaidJob = await Job.create(
        generateJob(pepeNunesContract.id, {
          paid: true,
          price: 10,
          paymentDate: onDate,
        })
    ...
    it("second is pepe botella", async () => {
      ...
      expect(result[1].toJSON().fullName).toEqual(
        `${pepeNunesClient.firstName} ${pepeNunesClient.lastName}`
      );
    });
