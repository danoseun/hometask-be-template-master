  ...
  describe("unpaidJobs", () => {
    describe("when there is no unpaid", () => {
      ...
      it("should get no one", async () => {
        const jobs = await Job.unpaidJobs();
    ...
    describe("when there is called with filters", () => {
      ...
      beforeAll(async () => {
        ...
        unpaidJob = await Job.create(
          generateJob(contract.id, { paid: false, description: "unpaid" })
        );
        anotherUnpaidJob = await Job.create(
          generateJob(contract.id, { paid: false })
        );
        paidJob = await Job.create(generateJob(contract.id, { paid: true }));
      });
      ...
      it("should get only one", async () => {
        const jobs = await Job.unpaidJobs({ where: { description: "unpaid" } });
        expect(jobs).toHaveLength(1);
      });
  ...
  describe("pay", () => {
    describe("when there is unpaid", () => {
      ...
      beforeAll(async () => {
        ...
        contract = await Contract.create(contractAttr);
        unpaidJob = await Job.create(
          generateJob(contract.id, { paid: false, paymentDate: null })
        );
        await unpaidJob.pay({});
